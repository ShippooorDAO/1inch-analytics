...
export function calculateDelta(a: number = 0, b: number = 0): number {
  return b - a;
}

export function calculateDeltaPercent(a: number = 0, b: number = 0) {
...
>(
  ...
  const zippedDatasets = zip(
    Array.from(Object.entries(datasetA)),
    Array.from(Object.entries(datasetB))
  );
  return {
